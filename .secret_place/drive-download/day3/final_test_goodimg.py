    ...
    for j in range(len(contours)):
        ...
        if w > 60 and h > 60 and w - h <= 7:
            ...
            if not same:
                ...
                blue_mask = cv2.inRange(marker, (100, 0, 0), (255, 100, 100))
                # pts1 = [[150, 0], [300, 0], [150, 150], [300, 150]]
                # for val in pts1:
                #     cv2.circle(marker, (val[0], val[1]), 5, (0, 255, 0), -1)
                mask = cv2.bitwise_or(green_mask, red_mask)
                mask = cv2.bitwise_or(blue_mask, mask)
                s1 = np.sum(mask[0:150, 0:150])
                s2 = np.sum(mask[150:300, 0:150])
                s3 = np.sum(mask[0:150, 150:300])
                s4 = np.sum(mask[150:300, 150:300])
                for ii in range(4):
                    if s3 < s1 or s3 < s2 or s3 < s4:
                        print('r'+str(ii))
