...
## 📡 Взаимодействие Роботов (Robot Communication)
Примеры того, как роботы могут общаться между собой или с центром управления.

### 1. HTTP API (FastAPI)
Современный способ управления роботом через веб-запросы.
*   **Сервер (Робот):** `src/communication/fastapi_robot.py`
*   **Клиент (ПК):** `src/communication/client_robot.py`

**Как это работает:**
Робот запускает веб-сервер. Вы отправляете ему команды через URL (например, `POST /move`).
```python
# Пример отправки команды на Python
requests.post("http://robot_ip:8000/move", json={"linear": 50, "angular": 0})
```

### 2. Обмен через файлы (Shared File)
Простой способ передачи данных между процессами на одном компьтере.
*   **Код:** `src/communication/shared_file.py`

**Как это работает:**
Один скрипт пишет команду в `commands.json`, другой скрипт постоянно проверяет этот файл.
Это удобно, если вы не хотите настраивать сеть.

---

## 🧠 Теория и Алгоритмы

### 🕹️ PID Регулятор (PID Controller)
Алгоритм для плавного управления (например, удержание полосы).

**Формула:**
$$u(t) = K_p e(t) + K_i \int e(t) dt + K_d \frac{de(t)}{dt}$$

*   **P (Proportional):** Чем больше ошибка (дальше от центра), тем сильнее поворачиваем.
    *   *Проблема:* Если ошибка маленькая, поворот слишком слабый, и мы никогда не доедем до идеального центра.
*   **I (Integral):** Накапливает ошибку со временем. Если мы долго едем чуть левее центра, I-компонента "надавит" и довернет нас.
*   **D (Derivative):** Предсказывает будущее. Если мы слишком быстро приближаемся к центру, D-компонента "притормозит" поворот, чтобы мы не проскочили мимо.
...
### ⚙️ Машина Состояний (State Machine)
Отличный способ управления сложным поведением (например, полет дрона).
Вместо кучи `if-else`, мы делим поведение на "Состояния".

**Пример (Дрон):**
1.  **IDLE** (Ожидание): Дрон на земле, моторы выключены.
2.  **TAKEOFF** (Взлет): Дрон набирает высоту до 1 метра.
3.  **HOVER** (Висение): Дрон держит позицию и ищет цель.
4.  **LAND** (Посадка): Дрон снижается.

```python
if state == "IDLE":
    if command == "start": state = "TAKEOFF"
elif state == "TAKEOFF":
    if altitude > 1.0: state = "HOVER"
elif state == "HOVER":
    track_object()
```
...
**OpenCV: Поиск цвета (HSV)**
```python
hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
mask = cv2.inRange(hsv, (0, 100, 100), (10, 255, 255)) # Красный
contours, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
```

**YOLO: Запуск на картинке**
```python
from ultralytics import YOLO
model = YOLO("yolov8n.pt")
results = model("image.jpg")
for r in results:
    boxes = r.boxes.xyxy # Координаты
```

**Math: Расстояние между точками**
```python
